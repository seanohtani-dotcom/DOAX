...
document.addEventListener("DOMContentLoaded", () => {
  const animateElements = document.querySelectorAll(
    ".product-card, .movie-card, .story-item, .gallery, .promo"
  );
      ...
      <div class="modal-body">
        ...
        <div class="modal-info">
          ...
          <div class="modal-actions">
            ...
            <button class="btn-secondary add-to-wishlist-modal">♥ Wishlist</button>
          </div>
        </div>
      </div>
    </div>
...
function openQuickView(product) {
  ...
  modal.querySelector(".add-to-wishlist-modal").onclick = () => {
  ...
  productCards.forEach((card) => {
    ...
    quickViewBtn.onclick = (e) => {
      ...
      const product = {
        ...
        price: card.querySelector(".price")?.textContent || "$0.00",
        description:
          card.querySelector("p")?.textContent || "No description available.",
      };
...
document.querySelectorAll(".character-slider > .character").forEach((c, i) => {
  ...
  rows.forEach((r) => {
    ...
    left.textContent = r[0];
    const middle = document.createElement("div");
    middle.textContent = r[1];
    const right = document.createElement("div");
    right.textContent = r[2];
  ...
  if (c.dataset.cv) {
    ...
    cvLine.className = "cv";
    cvLine.textContent = c.dataset.cv;
    info.appendChild(cvLine);
  } else {
    const cvLine = document.createElement("div");
    cvLine.className = "cv";
    cvLine.textContent = "CV: Minami Tsuda";
    info.appendChild(cvLine);
  }
  ...
  iconCol.className = "icon-column";
  const icons = [
    {
      symbol: "🔊",
      html: '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>',
    },
    {
      symbol: "🎵",
    ...
    {
      symbol: "📷",
      html: '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M9 2L7.17 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2h-3.17L15 2H9zm3 15c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z"/></svg>',
    },
  ];
  icons.forEach((icon) => {
  ...
  // right image
  const imgSrc = c.querySelector("img")
