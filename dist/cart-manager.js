...
class CartManager {
    constructor() {
        this.cart = [];
        this.STORAGE_KEY = "venusVacation_cart";
    ...
    async handleAddToCart(productId, button) {
        try {
            ...
            if (!product) {
                throw this.createError("PRODUCT_NOT_FOUND", `Product ${productId} not found`);
            }
            // Add to cart
    ...
    async handleBuyNow() {
        try {
            ...
            if (isNaN(quantity) || quantity < 1) {
                throw this.createError("INVALID_QUANTITY", "Invalid quantity specified");
            }
            // Get product data
            const product = await this.getProductData(productId);
            if (!product) {
            ...
            if (typeof window.showToast === "function") {
                window.showToast("Added to cart! Redirecting to checkout...", "success");
            }
            // Close modal
    ...
    async addItem(item) {
        ...
        if (existingIndex !== -1) {
            ...
            if (newQuantity > this.MAX_QUANTITY) {
                throw this.createError("MAX_QUANTITY_EXCEEDED", `Cannot add more than ${this.MAX_QUANTITY} of this item`);
            }
    ...
    async removeItem(itemId) {
        ...
        if (index === -1) {
            throw this.createError("ITEM_NOT_FOUND", `Item ${itemId} not found in cart`);
        }
    ...
    async updateQuantity(itemId, quantity) {
        if (quantity < 1 || quantity > this.MAX_QUANTITY) {
            throw this.createError("INVALID_QUANTITY", `Quantity must be between 1 and ${this.MAX_QUANTITY}`);
        }
    ...
    validateCartItem(item) {
        ...
        if (typeof item.quantity !== "number" ||
            ...
            throw this.createError("INVALID_ITEM", `Quantity must be between 1 and ${this.MAX_QUANTITY}`);
        }
    }
    /**
